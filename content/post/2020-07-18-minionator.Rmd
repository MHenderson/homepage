...
Minionator is an R package that generates constraint programs for the Minion constraint solver.

Here’s an example of a 3 x 3 latin square input file for Minion.

The input file can be used with Minion to find a latin square of order 3.

Call Minion from R (using system2) demo here.

It can also be used to enumerate (or even explicitly construct) all latin squares of order 3 (call from within R without using the rminion package).

Another demo of calling Minion from R.

Minionator helps you to use R to construct such input files. 
